"use client";

import React from "react";
import { useFormContext } from "react-hook-form";
import { Shield, Globe, AlertCircle } from "lucide-react";
import FormInput from "@/components/molecules/FormInput";
import FormCheckbox from "@/components/molecules/FormCheckbox";
import StatusIndicator from "@/components/features/admin/StatusIndicator";
import type { PaymentIntegration } from "@/types/settings";

interface PaymentSettingsFormProps {
  showSensitiveData: boolean;
  isLoading: boolean;
}

export default function PaymentSettingsForm({
  showSensitiveData,
  isLoading,
}: PaymentSettingsFormProps) {
  const form = useFormContext<PaymentIntegration>();
  const provider = form.watch("provider");
  const isActive = form.watch("isActive");

  return (
    <form className="space-y-8">
      {/* Status and Provider */}
      <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
        <div>
          <h4 className="text-lg font-medium text-black">
            Ödeme Sistemi Durumu
          </h4>
          <p className="text-sm text-gray-600">
            Ödeme entegrasyonunuzun aktiflik durumu
          </p>
        </div>
        <StatusIndicator isActive={isActive} isLoading={isLoading} />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Ödeme Sağlayıcısı
          </label>
          <select
            {...form.register("provider")}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black focus:border-black"
          >
            <option value="iyzico">Iyzico</option>
            <option value="stripe">Stripe</option>
            <option value="paytr">PayTR</option>
          </select>
        </div>

        <div className="flex items-center space-x-6 pt-6">
          <FormCheckbox name="isActive" label="Aktif" />
          <FormCheckbox name="isTestMode" label="Test Modu" />
        </div>
      </div>

      {/* Provider specific settings */}
      {provider === "iyzico" && (
        <div>
          <h4 className="text-lg font-medium text-black mb-4 flex items-center">
            <Shield className="h-5 w-5 mr-2" />
            Iyzico Ayarları
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormInput
              name="iyzicoApiKey"
              label="API Key"
              placeholder="API Key"
              type={showSensitiveData ? "text" : "password"}
            />
            <FormInput
              name="iyzicoSecretKey"
              label="Secret Key"
              placeholder="Secret Key"
              type={showSensitiveData ? "text" : "password"}
            />
            <FormInput
              name="iyzicoBaseUrl"
              label="API URL"
              placeholder="https://api.iyzipay.com"
            />
          </div>
        </div>
      )}

      {provider === "stripe" && (
        <div>
          <h4 className="text-lg font-medium text-black mb-4 flex items-center">
            <Shield className="h-5 w-5 mr-2" />
            Stripe Ayarları
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormInput
              name="stripePublishableKey"
              label="Publishable Key"
              placeholder="pk_test_..."
              type={showSensitiveData ? "text" : "password"}
            />
            <FormInput
              name="stripeSecretKey"
              label="Secret Key"
              placeholder="sk_test_..."
              type={showSensitiveData ? "text" : "password"}
            />
          </div>
        </div>
      )}

      {provider === "paytr" && (
        <div>
          <h4 className="text-lg font-medium text-black mb-4 flex items-center">
            <Shield className="h-5 w-5 mr-2" />
            PayTR Ayarları
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormInput
              name="paytrMerchantId"
              label="Merchant ID"
              placeholder="Merchant ID"
              type={showSensitiveData ? "text" : "password"}
            />
            <FormInput
              name="paytrMerchantKey"
              label="Merchant Key"
              placeholder="Merchant Key"
              type={showSensitiveData ? "text" : "password"}
            />
            <FormInput
              name="paytrMerchantSalt"
              label="Merchant Salt"
              placeholder="Merchant Salt"
              type={showSensitiveData ? "text" : "password"}
            />
          </div>
        </div>
      )}

      {/* Callback URLs */}
      <div>
        <h4 className="text-lg font-medium text-black mb-4 flex items-center">
          <Globe className="h-5 w-5 mr-2" />
          Callback URL'leri
        </h4>
        <div className="grid grid-cols-1 gap-6">
          <FormInput
            name="callbackUrl"
            label="Başarılı Ödeme URL"
            placeholder="https://fotomandalin.com/payment/success"
          />
          <FormInput
            name="webhookUrl"
            label="Webhook URL"
            placeholder="https://fotomandalin.com/api/webhooks/payment"
          />
        </div>
      </div>

      {!showSensitiveData && (
        <div className="flex items-start space-x-3 p-4 bg-amber-50 rounded-lg border border-amber-200">
          <AlertCircle className="h-5 w-5 text-amber-600 mt-0.5" />
          <div className="text-sm text-amber-800">
            <p className="font-medium">Hassas Veriler Gizli</p>
            <p>
              API anahtarları ve diğer hassas veriler güvenlik amacıyla
              gizlenmiştir. Görmek için sağ üstteki düğmeyi kullanın.
            </p>
          </div>
        </div>
      )}
    </form>
  );
}
